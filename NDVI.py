...
def NDVICalc(original):
    ...
    ndvi2 = ((red-blue)/summ).astype('float')
    ndvi2[ndvi2<0] = 0
    
    
    #for x in range(oldHeight):
        #print ndvi2[x,1]+ +ndvi2[x,10]+ +ndvi2[x,x]+ +ndvi2[x,50]
     
    ndvi_value = 0
    
    for x in range(oldHeight):
        for y in range(oldWidth):
            ndvi_value = ndvi_value + ndvi2[x,y] 
    ...
    #real_ndvi = (redSat - bluSat) / (redSat + bluSat)
    #real_ndvi = (bluSat - redSat) / (redSat + bluSat)
